...
_MACRO_RULES = [
    # (market, title, importance, rule)  importance: 3=최상 2=상 1=중
    ("US", "고용보고서(비농업 고용)", 3, lambda y, m: _nth_weekday(y, m, _FRI, 1)),
    ("US", "소비자물가(CPI)", 3, lambda y, m: _nth_weekday(y, m, _WED, 2)),
    ("US", "생산자물가(PPI)", 2, lambda y, m: _nth_weekday(y, m, _THU, 2)),
    ("US", "소매판매", 2, lambda y, m: _nth_weekday(y, m, _TUE, 3)),
    ("US", "개인소비지출(PCE) 물가", 2, lambda y, m: _last_weekday(y, m, _FRI)),
    ("US", "ISM 제조업 PMI", 1, lambda y, m: _clamp_day(y, m, 1)),
    ("KR", "수출입동향(관세청)", 2, lambda y, m: _clamp_day(y, m, 1)),
    ("KR", "소비자물가동향(통계청)", 3, lambda y, m: _nth_weekday(y, m, _TUE, 1)),
]

# 분기 지표 (해당 분기 첫 달에 전분기치 발표)
_QUARTERLY = [
    ("US", "GDP(속보치)", 2, {1: 30, 4: 30, 7: 30, 10: 30}),
    ("KR", "GDP(속보치, 한국은행)", 2, {1: 25, 4: 25, 7: 25, 10: 25}),
]
...
def macro_events(year: int, month: int) -> list[dict]:
    ...
    def add(market: str, title: str, importance: int, d: date | None):
    ...
    for market, title, imp, rule in _MACRO_RULES:
        add(market, title, imp, rule(year, month))

    for market, title, imp, month_day in _QUARTERLY:
        if month in month_day:
            add(market, title, imp, _clamp_day(year, month, month_day[month]))

    for iso in _FOMC_2026:
        d = date.fromisoformat(iso)
        add("US", "FOMC 정책금리 결정", 3, d)
    for iso in _BOK_2026:
        d = date.fromisoformat(iso)
        add("KR", "한국은행 금통위 정책금리 결정", 3, d)
