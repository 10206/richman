...
## A-P4-3. KR 전력 117460은 yfinance에서 정상 조회됨 — 스킵 불필요

- 우려했던 `117460.KS`(KR 전력 프록시)는 yfinance에서 데이터가 나옴 → 섹터 스킵 없이
  KR 6개 섹터 전부 백필 성공.
- 단, 일부 KR ETF는 상장 이력이 짧아 (로봇 445290: 2023-11 상장 등) 백필 시작일이
  섹터마다 다름 (KR 최초 저장일 2024-04-22, 워밍업 제외 후). docs/07의
  "전력 = 유틸리티인지 전력기기인지" 사용자 확정 항목은 여전히 유효.
