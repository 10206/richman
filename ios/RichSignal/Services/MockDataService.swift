...
final class MockDataService: SignalDataProviding {
    ...
    func calendar(month: String?) async throws -> CalendarResponse {
        ...
        var events = [
            ...
            ev(21, .US, .macro, "소매판매", 2, true, "08:30 ET"),
            ev(23, .US, .earnings, "Intel (INTC) 실적", 2, true, "장 마감 후", .beat),
            ev(28, .US, .earnings, "Texas Instruments (TXN) 실적", 2, true, "장 마감 후"),
            ev(29, .US, .macro, "FOMC 정책금리 결정", 3, true, "14:00 ET"),
        ]
        // 오늘 일정 하나 보장 (강조·발표시간 데모)
        events.append(ev(todayDay, .US, .macro, "개인소비지출(PCE) 물가", 2, true, "08:30 ET"))
